  ...
  if (existingUser)
    return response.status(422).json({
      ...
      responseMessage : `User with this email ${
          email} already exist , try again with another email`,
    });
...
exports.login = async (request, response) => {
  ...
  if (!userEmail)
    respons.status(422).json({
      success : "Failed",
      responseMessage :
          `No user with this ${userEmail} , please try with a differnt email`,
    });

  try {
    userEmail && Bcrypt.compareSync(request.body.email, userEmail.email);
    const loginToken =
        Jwt.sign({userEmail : userEmail.email}, process.env.PAYLOAD_SECRET);
    return response.status(200).json(
        {success : true, token : loginToken, user_email : userEmail.email});
  } catch (error) {
    return response.status(422).json({
      success : false,
